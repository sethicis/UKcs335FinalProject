...
sub writeOutput {
	...
	#MODIFIED: Changed import command for java friendliness
	...
	#MODIFIED added for java compatibility
	print OUTFILE "public class ".$object." {\n\n";
	# needed constant for glDrawArrays
	#print OUTFILE "unsigned int ".$object."NumVerts = ".($numFaces * 3).";\n\n";
	# Modified for opengl es android program
	print OUTFILE "public static int ".$object."NumVerts = ".($numFaces * 3).";\n\n";
	
	# write verts
	#MODIFIED
	print OUTFILE "public static float ".$object."Verts \[\] = {\n";
	for( $j = 0; $j < $numFaces; $j++)
	{
		...
		#MODIFIED: Added an 'f' after each number for java friendliness
	...
	# write normals
	if($numNormals > 0) {
		print OUTFILE "public static float ".$object."Normals \[\] = {\n";
		for( $j = 0; $j < $numFaces; $j++) {
	...
	if($numTexture) {
		print OUTFILE "public static float ".$object."TexCoords \[\] = {\n";
		for( $j = 0; $j < $numFaces; $j++) {
			...
			print OUTFILE "  // $face_line[$j]\n";
			#MODIFIED: Added an 'f' after each number for java friendliness
			print OUTFILE "  $tx[$ia]f, $ty[$ia]f,\n";
			print OUTFILE "  $tx[$ib]f, $ty[$ib]f,\n";
			print OUTFILE "  $tx[$ic]f, $ty[$ic]f,\n";
		}
